...
void Leaf::drawTest()
{
	...
	// is there a way to store the points reversed points so that the fractal can be one object?
	//for(int i = splitLeafPointsReverse.size(); i > 0; i--)
	//{
		//splitLeafPoints.push_back(vec3(splitLeafPointsReverse[i].x, splitLeafPointsReverse[i].y, splitLeafPointsReverse[i].z));
	//}
	glRotatef(30.0f, 0.0, 0.0, 1.0);
	glBegin(GL_POLYGON);
	for(int i = 1; i < splitLeafPoints.size() - stopLeaf; i++)
	{
		glColor3f(0.58f, 0.80f, 0.11f);
