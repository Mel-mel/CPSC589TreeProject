...
void Tree::drawTree()
{
	...
	glBegin(GL_LINES);
	glColor3f(0.0f, 0.5f, 0.8f);
	for(int i = 0; i < /*allPoints.size()*/ 10; i++)
	{
		for(int j = 0; j < removable.size()+10; j++)
		{
			float x = allPoints[i].x - removable[j].x;
			float y = allPoints[i].y - removable[j].y;
			float z = allPoints[i].z - removable[j].z;
			...
			//distlength = sqrt(distlength);
			//cout << distlength<< endl;
			//1.5f > distlength
...
void Tree::spaceAlgorithm()
{
	...
	for (int k = 0; k < allPoints.size(); k++)
	{
		...
		removable.push_back(allPoints[k]);
	}
