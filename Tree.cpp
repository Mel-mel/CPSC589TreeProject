...
{	
	...
	if(n == num)
	{
		...
		leftTemp.x = (leftTemp.x - x_change)*cos(M_PI/6);
		//leftTemp.y = leftTemp.y + y_change;
		leftTemp.y = (leftTemp.y + y_change)*sin(M_PI/6);
		splitPoints.push_back(leftTemp);
		
		//Right branch
		vec2 rightTemp = splitPoints[n];
		rightTemp.x = (rightTemp.x + x_change)*cos(-M_PI/6);
		rightTemp.y = (rightTemp.y +  y_change)*sin(M_PI/6);
