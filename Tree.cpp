...
float theta = M_PI/6;
float temp = 0.5f;
int numOfBranches = 100;

//Values to change how the branches are drawn
float x_change = 0.1f;
float y_change = 0.1f;

float radiusInfluence = 8.0f;	//If a trunk node has a close neighbor to some branch point
...
void Tree::genRandomBranch()
{
    ...
    for (int i = 0; i < numOfBranches; i++)
    {
        ...
        randY = ((float)rand() / (float)(RAND_MAX)) * maxValue;
...
void Tree::drawTree()
{
	...
	{
		glVertex3f(treeNodes[i].position.x, treeNodes[i].position.y, treeNodes[i].position.z);
	...
	for(int p = 0; p < 6; p++)
	{
		glVertex3f(treeNodes[p].position.x, treeNodes[p].position.y, treeNodes[p].position.z);
		glVertex3f(treeNodes[p+1].position.x, treeNodes[p+1].position.y, treeNodes[p+1].position.z);
	}
	glEnd();
	...
	glBegin(GL_LINES);
	glColor3f(0.0f, 0.5f, 0.8f);
	for(int i = 0; i < treeNodes.size()-6; i++)
	{
		for(int j = 6; j > removable.size(); j++)
		{
			if(count == treeNodes[i].group)
			{
				glVertex3f(treeNodes[i].position.x, treeNodes[i].position.y, treeNodes[i].position.z);
				glVertex3f(removable[j].x, removable[j].y, removable[j].z);
			}
		}
		count++;
...
void Tree::removeBranches()
{
	...
	for (int i = 0; i < treeNodes.size(); i++)
	{
		for (int j = 0; j < randBranch.size(); j++)
		{
			...
			x = treeNodes[i].position.x - randBranch[j].x;
			y = treeNodes[i].position.y - randBranch[j].y;
			z = treeNodes[i].position.z - randBranch[j].z;
...
void Tree::spaceAlgorithm()
{
	...
	for(i = 0; i < treeNodes.size(); i++)
	{
		for(j = 0; j < randBranch.size(); j++)
		{
			...
			{	
				neighbors.push_back(treeNodes[i].position);
		...
		if(!neighbors.empty())
		{
			...
			norm = treeNodes[i].position + (1.5f * norm);
			/*cout << endl;
	...
	for (int k = 0; k < treeNodes.size(); k++)
	{
		cout << endl;
		cout << "index k " << k << endl;
		cout << treeNodes[k].position.x << " " << treeNodes[k].position.y << " " << treeNodes[k].position.z << endl;
		removable.push_back(treeNodes[k].position);
	}
